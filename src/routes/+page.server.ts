...
export const actions = {
	incomeTax: async ({ request }) => {
		...
		const incomeTax = data.get('incomeTax');
		...
		const wageTaxMatch = new RegExp(/<ausgabe name="LSTLZZ" value="(\d+)" type="STANDARD"\/>/).exec(xmlData);
		const wageTax = wageTaxMatch ? Number(wageTaxMatch[1]) / 100 : null; // convert from cents to euros

		const usedAllowanceMatch = new RegExp(/<ausgabe name="VFRB" value="(\d+)" type="DBA"\/>/).exec(xmlData);
